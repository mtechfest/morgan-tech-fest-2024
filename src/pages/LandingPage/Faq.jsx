...
const Faq = () => {
  const [isOpen, setIsOpen] = useState(Array(16).fill(false));
  ...
  const faqData = [
    {
      ...
      answer: "Morgan TechFest is a one-day in-person student-led tech conference that brings together developers, researchers, and tech enthusiasts across universities in the US.",
    },
    {
    ...
    },
    {
      ...
      answer: "All tech-related projects including research (AI, VR, Blockchain, Software Development, Robotics etc.)",
    },
    {
    ...
    },
    {
      ...
      answer: "It is a one-day event on Saturday September 28, 2024 from 10am-6pm",
    },
    {
      question: "Are provisions like breakfast, lunch, and dinner included?",
      answer: "Yes! Food and drinks are available, and we also do our best to accommodate all kinds of dietary restrictions",
    },
    {
      question: "How do I get there?",
      answer: "We may provide transportation support through your university, department, or student organization.",
    },
    {
    ...
    {
      question: "Location?",
      answer: "Morgan State University Student Center - Calvin & Tina Tyler Ballroom. Parking is available in the Morgan Commons Garage",
    },
    {
  ...
  return (
    <div className="bg-[#F1ECF1]" name='faqs'>
        ...
        {faqData.map((faq, index) => (
          <div key={index} className="mb-10">
            <div className="flex items-center justify-between gap-9 md:gap-4 mb-2 text-[#122644]">
              ...
              <button
                className="w-10 h-10 min-w-[40px] border border-[#122644] rounded-full flex items-center justify-center mr-3 cursor-pointer hover:bg-[#122644] hover:text-white transition-colors"
                onClick={() => toggleQuestion(index)}
                aria-expanded={isOpen[index]}
                aria-label={isOpen[index] ? "Collapse answer" : "Expand answer"}
              >
                {isOpen[index] ? <FaArrowUp /> : <FaArrowDown />}
              </button>
            </div>
            
            {/* OPTIMIZATION: Smooth transition for expand/collapse */}
            <div 
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                isOpen[index] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
              }`}
            >
              <p className="text-sm pr-8 pt-2 text-gray-700">{faq.answer}</p>
            </div>
          </div>
        ))}
      </div>
